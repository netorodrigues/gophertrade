...
type Config struct {
	Web struct {
		APIHost string `conf:"default:0.0.0.0:8082,env:ORDER_HTTP_PORT"`
	}
	GRPC struct {
		InventoryAddr string `conf:"default:localhost:9091,env:INVENTORY_GRPC_ADDR"`
	}
	DB struct {
		URL string `conf:"default:postgres://postgres:postgres@localhost:5432/order?sslmode=disable,env:DATABASE_URL"`
	}
	Kafka struct {
